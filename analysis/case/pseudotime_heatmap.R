pseudotime_heatmap <- function (cds_subset, cluster_rows = TRUE, hclust_method = "ward.D2", 
          ...
          cores = 1) 
{
  ...
  if (is.null(hmcols)) {
    ...
    hmcols <- colorRampPalette(rev(brewer.pal(11, "RdBu")))(length(bks) - 1)
  }
  else {
  ...
  if (return_heatmap) {return(list(heatmap_matrix = heatmap_matrix, annotation_row = annotation_row,ph_res = ph_res))}
}
