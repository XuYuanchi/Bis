...
cal_metric <- function(count, label){
  ...
  x.seurat <- FindClusters(x.seurat, resolution = 0.01)
  # 1:0.7
  # 2:0.3
  # 3:
...
h5file=H5Fopen('/home/suyanchi/project/dab/results/1M/bis_4096.h5')
dat = h5file$data
metric = cal_metric(as.matrix((dat)), label)
saveRDS(file=paste0(dirs, 'metric_bis_4096.rds'), metric)
h5closeAll()
print(metric[1:3])
print('bis done')